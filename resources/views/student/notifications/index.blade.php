    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow mb-6">
            ...
            <div class="px-6 py-3 bg-gray-50 flex flex-wrap gap-2">
                ...
                <a href="?category=payments&status={{ request('status', 'all') }}"
                    class="@if(request('category') === 'payments') bg-indigo-600 text-white @else bg-white text-gray-700 @endif px-3 py-1 rounded-full text-sm border border-gray-300 hover:bg-indigo-50">
                    Payments
                </a>
                <a href="?category=certificates&status={{ request('status', 'all') }}"
                    class="@if(request('category') === 'certificates') bg-indigo-600 text-white @else bg-white text-gray-700 @endif px-3 py-1 rounded-full text-sm border border-gray-300 hover:bg-indigo-50">
                    Certificates
                </a>
            </div>
        </div>
        ...
        @if($notifications->count() > 0)
            <div class="bg-white rounded-lg shadow p-4 mb-6 flex items-center justify-between">
                ...
                @if(auth()->user()->unreadNotifications->count() > 0)
                    <form action="{{ route('student.notifications.readAll') }}" method="POST" class="inline">
                        @csrf
