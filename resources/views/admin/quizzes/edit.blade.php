                ...
                <div class="lg:col-span-2 space-y-6">
                            ...
                            <div>
                                ...
                                <select name="course_id" id="course_id" required
                                    ...
                                    @foreach($courses as $course)
                                        <option value="{{ $course->id }}" data-slug="{{ $course->slug }}" {{ old('course_id', $quiz->course_id) == $course->id ? 'selected' : '' }}>
                                            {{ $course->title }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="bg-white shadow-md rounded-lg overflow-hidden">
                        ...
                        <div class="p-6 space-y-6">
                            <!-- Description -->
                            <div>
                                <x-quill-editor name="description" :value="old('description', $quiz->description)"
                                    label="Description" :required="false" height="200px"
                                    placeholder="Describe the quiz content and objectives..." />
                            </div>

                            <!-- Instructions -->
                            <div>
                                <x-quill-editor name="instructions" :value="old('instructions', $quiz->instructions)"
                        ...
                        <div class="p-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <label for="order" class="block text-sm font-medium text-gray-700 mb-1">Display
                                        Order</label>
                                    <input type="number" name="order" id="order" value="{{ old('order', $quiz->order) }}"
                                        min="0"
                                        class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition duration-150">
                                    @error('order')
                                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div class="mt-6">
                                ...
                                <select name="show_answers" id="show_answers" required
                                    class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition duration-150">
                                    <option value="never" {{ old('show_answers', $quiz->show_answers) == 'never' ? 'selected' : '' }}>Never</option>
                                    <option value="after_submission" {{ old('show_answers', $quiz->show_answers) == 'after_submission' ? 'selected' : '' }}>After Submission
                                    </option>
                                    <option value="after_passing" {{ old('show_answers', $quiz->show_answers) == 'after_passing' ? 'selected' : '' }}>After Passing</option>
                                    <option value="always" {{ old('show_answers', $quiz->show_answers) == 'always' ? 'selected' : '' }}>Always</option>
                                </select>
                                @error('show_answers')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                ...
                <div class="lg:col-span-1 space-y-6">
                    ...
                    <div class="bg-white shadow-md rounded-lg overflow-hidden sticky top-6">
                        ...
                        <div class="p-6 space-y-4">
                            ...
                            <div class="flex items-start p-3 rounded-lg hover:bg-gray-50 transition duration-150">
                                <div class="flex items-center h-5 mt-0.5">
                                    <input type="checkbox" name="require_passing" id="require_passing" value="1" {{ old('require_passing', $quiz->require_passing) ? 'checked' : '' }}
                                        class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                </div>
                                <div class="ml-3">
                            ...
                            <div class="flex items-start p-3 rounded-lg hover:bg-gray-50 transition duration-150">
                                <div class="flex items-center h-5 mt-0.5">
                                    <input type="checkbox" name="certificate_eligible" id="certificate_eligible" value="1"
                                        {{ old('certificate_eligible', $quiz->certificate_eligible) ? 'checked' : '' }}
                                        class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                </div>
                                <div class="ml-3">
    ...
    @push('scripts')
        <script>
            function loadTopics(selectElement) {
                ...
                const currentTopicId = "{{ old('topic_id', $quiz->topic_id) }}";
