...
<div class="max-w-7xl mx-auto">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.courses.show', $course) }}" class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition ease-in-out duration-150">
                        ...
                        <div>
                            ...
                            <select name="course_categories[]" id="course_categories" multiple class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" size="4">
                                @foreach($courseCategories as $courseCategory)
                                    <option value="{{ $courseCategory->id }}" {{ in_array($courseCategory->id, old('course_categories', $course->courseCategories->pluck('id')->toArray())) ? 'selected' : '' }}>
                                        {{ $courseCategory->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="course_tags[]" id="course_tags" multiple class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" size="4">
                                @foreach($courseTags as $courseTag)
                                    <option value="{{ $courseTag->id }}" {{ in_array($courseTag->id, old('course_tags', $course->courseTags->pluck('id')->toArray())) ? 'selected' : '' }}>
                                        {{ $courseTag->name }}
                                    </option>
                                @endforeach
                            </select>
                       ...
                       <div class="space-y-3 border-t pt-4">
                            ...
                            <label class="flex items-start">
                                <input type="checkbox" name="auto_enroll_enabled" value="1" 
                                       {{ old('auto_enroll_enabled', $course->auto_enroll_enabled) ? 'checked' : '' }} 
                                ...
                                <div class="ml-2">
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">
                                        ...
                                        When disabled, all auto-enrollments will be removed.
                                    </p>
                                </div>
                            </label>
                        </div>
    ...
    <!-- Auto-Enrollment Progress Modal -->
    <div id="enrollmentProgressModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-medium text-gray-900" id="modalTitle">Processing Auto-Enrollment</h3>
                ...
                <div id="progressDetails" class="text-sm text-gray-600 mb-4">
                    <p id="detailsText">Preparing to process students...</p>
                </div>
                
                <div id="completionMessage" class="hidden mb-4">
                    <div class="rounded-md bg-green-50 p-4">
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                <p class="text-sm font-medium text-green-800" id="completionText"></p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end">
                    <button onclick="closeProgressModal()" id="closeButton" class="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        Close
                    </button>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    @if(session('success') && (str_contains(session('success'), 'background') || str_contains(session('success'), 'Auto-enrollment') || str_contains(session('success'), 'Auto-unenrollment')))
        // Auto-enrollment/unenrollment was just triggered, start checking progress
        setTimeout(() => {
            showProgressModal();
            startProgressCheck();
        }, 500);
...
function showProgressModal() {
    document.getElementById('enrollmentProgressModal').classList.remove('hidden');
    isProcessing = true;
}

function closeProgressModal() {
    document.getElementById('enrollmentProgressModal').classList.add('hidden');
    if (progressInterval) {
...
function startProgressCheck() {
    ...
    progressInterval = setInterval(checkProgress, 1500); // Check every 1.5 seconds
}

async function checkProgress() {
    try {
        const response = await fetch('{{ route('admin.courses.enrollment-progress', $course) }}', {
            method: 'GET',
            headers: {
        ...
        const typeLabel = data.type === 'enrollment' ? 'Enrolling' : 'Unenrolling';
        
        // Update modal title
        document.getElementById('modalTitle').textContent = `Processing Auto-${data.type === 'enrollment' ? 'Enrollment' : 'Unenrollment'}`;
        ...
        progressText.textContent = `${typeLabel} students...`;
        detailsText.textContent = `${progress.completed} out of ${progress.total} students processed`;
        ...
        if (progress.status === 'completed') {
            ...
            document.getElementById('completionText').textContent = 
                `Successfully ${data.type === 'enrollment' ? 'enrolled' : 'unenrolled'} ${progress.completed} students!`;
            
            // Auto-close after 3 seconds
            setTimeout(() => {
                closeProgressModal();
                // Redirect to course show page
                window.location.href = '{{ route('admin.courses.show', $course) }}';
            }, 3000);
            ...
            document.getElementById('errorText').textContent = 
                `${data.type === 'enrollment' ? 'Enrollment' : 'Unenrollment'} process failed. Please try again.`;
        }
