...
<div class="space-y-6">
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            ...
                            <dd class="text-2xl font-semibold text-gray-900">{{ $categories->total() }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            ...
                            <dd class="text-2xl font-semibold text-gray-900">{{ $activeCount ?? 0 }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            ...
                            <dd class="text-2xl font-semibold text-gray-900">{{ $totalCourses ?? 0 }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            ...
                            <dd class="text-2xl font-semibold text-gray-900">{{ $parentCount ?? 0 }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white p-4 rounded-lg shadow">
        <form method="GET" action="{{ route('admin.categories.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search categories..." class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            </div>
            ...
            <div>
                <select name="parent" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    <option value="">All Types</option>
                    <option value="parent" {{ request('parent') == 'parent' ? 'selected' : '' }}>Parent Only</option>
                    <option value="child" {{ request('parent') == 'child' ? 'selected' : '' }}>Sub-categories</option>
                </select>
            </div>

            <div class="flex space-x-2">
                <button type="submit" class="flex-1 bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700">Filter</button>
                <a href="{{ route('admin.categories.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">Reset</a>
            </div>
        </form>
    </div>
                ...
                @forelse($categories as $category)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                @if($category->icon)
                                    <div class="flex-shrink-0 h-10 w-10 rounded-lg bg-indigo-100 flex items-center justify-center text-2xl">
                                        {{ $category->icon }}
                                    </div>
                                @else
                                ...
                                <div class="ml-4">
                                    ...
                                    @if($category->description)
                                        <div class="text-sm text-gray-500 truncate max-w-xs">{{ Str::limit($category->description, 50) }}</div>
                                    @endif
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            @if($category->parent)
                                <span class="text-indigo-600">{{ $category->parent->name }}</span>
                            @else
