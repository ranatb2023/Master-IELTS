...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-white p-4 rounded-lg shadow">
            <form method="GET" action="{{ route('tutor.topics.all') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <!-- Search -->
                <div>
                        ...
                        @forelse($topics as $topic)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($topic->course)
                                        <a href="{{ route('tutor.courses.show', $topic->course) }}"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-3">
                                        @can('course.view')
                                            <a href="{{ route('tutor.topics.show', $topic) }}"
                                        ...
                                        @endcan
                                        @can('course.delete')
                                            <form action="{{ route('tutor.courses.topics.destroy', [$topic->course, $topic]) }}"
                                                method="POST" class="inline"
                                                onsubmit="return confirm('Are you sure you want to delete this topic? This will also delete all lessons, quizzes, and assignments in this topic.');">
                                                @csrf
                                                @method('DELETE')
